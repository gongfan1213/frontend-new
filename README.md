...
Nexus is an open source web application for collaborating with agents to better write documents. It is inspired by [OpenAI's "Canvas"](https://openai.com/index/introducing-canvas/), but with a few key differences.
...
2. **Built in memory**: Nexus ships out of the box with a [reflection agent](https://langchain-ai.github.io/langgraphjs/tutorials/reflection/reflection/) which stores style rules and user insights in a [shared memory store](https://langchain-ai.github.io/langgraphjs/concepts/memory/). This allows Nexus to remember facts about you across sessions.
3. **Start from existing documents**: Nexus allows users to start with a blank text, or code editor in the language of their choice, allowing you to start the session with your existing content, instead of being forced to start with a chat interaction. We believe this is an ideal UX because many times you will already have some content to start with, and want to iterate on-top of it.

## Features

- **Memory**: Nexus has a built in memory system which will automatically generate reflections and memories on you, and your chat history. These are then included in subsequent chat interactions to give a more personalized experience.
...
- **Live markdown rendering & editing**: Nexus's markdown editor allows you to view the rendered markdown while you're editing, without having to toggle back and fourth.

## Setup locally

This guide will cover how to setup and run Nexus locally. If you prefer a YouTube video guide, check out [this video](https://youtu.be/sBzcQYPMekc).

### Prerequisites

Nexus requires the following API keys and external services:
...
The first step to running Nexus locally is to build the application. This is because Nexus uses a monorepo setup, and requires workspace dependencies to be build so other packages/apps can access them.
...
After your LangGraph server is running, execute the following command inside `apps/web` to start the Nexus frontend:
...
Nexus is designed to be compatible with any LLM model. The current deployment has the following models configured:
...
Nexus supports calling local LLMs running on Ollama. This is not enabled in the hosted version of Nexus, but you can use this in your own local/deployed Nexus instance.
...
Below are some common issues you may run into if running Nexus yourself:
...
Below is a list of features we'd like to add to Nexus in the near future:

- **Render React in the editor**: Ideally, if you have Nexus generate React (or HTML) code, we should be able to render it live in the editor. **Edit**: This is in the planning stage now!
...
We'd like to continue developing and improving Nexus, and want your help!
